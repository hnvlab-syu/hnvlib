...
def run_pytorch_lightning(batch_size: int, epochs: int, lr: float) -> None:
    ...
    model = FashionMNISTNetworkModule(lr=lr)
    wandb_logger = WandbLogger()
    trainer = pl.Trainer(max_epochs=epochs, accelerator='gpu', devices=1, logger=wandb_logger)
    trainer.fit(model, train_dataloaders=train_dataloader, val_dataloaders=test_dataloader)

    trainer.save_checkpoint('fashion-mnist-net.ckpt')
    print('Saved PyTorch Lightning Model State to fashion-mnist-net.ckpt')

    model = FashionMNISTNetworkModule.load_from_checkpoint(checkpoint_path='fashion-mnist-net.ckpt')
